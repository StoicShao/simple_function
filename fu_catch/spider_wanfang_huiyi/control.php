			...
			if($res){
				...
				for($i=$value;$i<=$total_page;$i++){
					...
					for($k=0;$k<$articles_num;$k++){
						...
						$data['jg']	 	 = $jg;
						//会议名称
						$data['conf_name']	 = $this->chulistring3($articles[$k]['conf_name']);
						//会议时间
						//$data['startdate_char']	= $this->chulistring3($articles[$k]['startdate_char']);
						if($articles[$k]['pro_pub_date']['time']){
							$data['startdate_char']= date('Y-m-d H:i:s',$this->chulistring3($articles[$k]['pro_pub_date']['time'])/1000);
						}else{
							$data['startdate_char']= '';
						}

						//会议地点
						$data['conf_place']	    = $this->chulistring3($articles[$k]['conf_place']);
						//会议主办单位
						if(count($articles[$k]['hostunit_name']) >= 2){
							$hostunit_name 	 = '';
							for($kk=0;$kk<count($articles[$k]['hostunit_name']);$kk++){
								$hostunit_name.=  $this->chulistring3($articles[$k]['hostunit_name'][$kk]).'@@@';
							}
						}else{
							$hostunit_name	  = $this->chulistring3($articles[$k]['hostunit_name']);
						}
						$data['hostunit_name'] = str_replace('%','@@@',$hostunit_name);
						//出处
						$data['provenance']    = '';

						//母体文献
						$data['mother_literature']	 = $this->chulistring3($articles[$k]['mother_literature']);
						...
						//年，卷期
						$data['year']	 = $this->chulistring3($articles[$k]['conf_year']);
		...
		if($arr['complete_count'] < $arr['total_count']){
			...
			for($z	= 0;$z<=$lastid;$z+=$step)
			{
				...
				while($row	= mysql_fetch_assoc($result))
				{
					...
					$strstring	.='conf_name-会议名称:'.$row['conf_name']."\r\n";//
					$strstring	.='startdate_char-会议时间:'.$row['startdate_char']."\r\n";//
					$strstring	.='conf_place-会议地点:'.$row['conf_place']."\r\n";//
					$strstring	.='hostunit_name-会议主办单位:'.$row['hostunit_name']."\r\n";//
					$strstring	.='provenance-出处:'.$row['provenance']."\r\n";//

					$strstring	.='mother_literature-母体文献:'.$row['mother_literature']."\r\n";//
